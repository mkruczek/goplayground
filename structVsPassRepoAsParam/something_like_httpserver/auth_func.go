...
func AuthenticateMiddleware(repo DeviceRepository) gin.HandlerFunc {
	return func(c context.Context) {
		...
		device, err := repo.GetDeviceByTokenId(c, tokenId)

		devices, _ := repo.GetDevicesByTokenIds(c, []string{tokenId})
		fmt.Sprintf("%v", devices)

		repo.InsertDevice(c, Device{
		...
		_, err = Authenticate(c, "", token, device.TokenPrivKey)
		if err != nil {
			panic(err)
		}
	}
...
func Authenticate(ctx context.Context, tokenId string, token string, tokenPrvKeyBytes []byte) (ctxWithAuthentication context.Context, err error) {
	authBearer, err := decodeToken(ctx, token, tokenPrvKeyBytes)
	if err != nil {
		//TODO
		return nil, err
	}
...
func decodeToken(ctx context.Context, token string, tokenPrvKeyBytes []byte) (authBearer string, err error) {
